...
proc myTrace {file} {
    ...
    #puts  $qlength "$now [expr $cwnd(0)*$N-$RTT*10000*8/$traceSamplingInterval/1000000]"
   ...
   # puts -nonewline $file "$pdepartures_"  
...
for {set i 0} {$i < $N} {incr i} {
    ...
    $ns at [expr 0.1 + 1.0 * $i / ($N + 0.0001)] "$ftp($i) start"     
   # $ns at [expr $simulationTime] "$ftp($i) stop"
   set x [expr $N-$i-1]
    $ns at [expr 0.91+($i+1)*0.2] "$ftp($x) stop"
}
